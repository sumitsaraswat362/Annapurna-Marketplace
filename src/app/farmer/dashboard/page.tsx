...
function ListingModal({ onClose }: { onClose: () => void }) {
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPhoto(file);
      // Convert to base64 data URL so it persists in Firestore
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Price validation: max 3x mandi price
    if (mandiPrice && Number(askingPrice) > mandiPrice * 3) {
      alert(`Price too high! Current mandi price is ₹${mandiPrice}/kg. Maximum allowed: ₹${Math.round(mandiPrice * 3)}/kg`);
      return;
    }
    ...
    try {
      ...
      // Don't dispatch ADD_LISTING locally — Firestore onSnapshot will auto-sync it
      setShowToast(true);
      setTimeout(() => {
        onClose();
      }, 1500);
