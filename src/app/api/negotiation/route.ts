...
export async function POST(req: Request) {
  try {
    ...
    const listingId = body.listingId || `${cropType}-${variety}`;
    const oneHourAgo = Date.now() - (60 * 60 * 1000);

    try {
      // Check bids per listing per buyer
      const perListingSnap = await firestore.collection('negotiation_history')
        .where('buyerId', '==', buyerId)
        .where('listingId', '==', listingId)
        .get();
      
      if (perListingSnap.size >= MAX_BIDS_PER_LISTING) {
        return NextResponse.json({ 
          error: `Maximum ${MAX_BIDS_PER_LISTING} negotiation rounds per listing reached. Please try a different approach.`,
          action: 'rate_limited'
        }, { status: 429 });
      }

      // Check total bids per hour
      const perHourSnap = await firestore.collection('negotiation_history')
        .where('buyerId', '==', buyerId)
        .where('timestamp', '>=', oneHourAgo)
        .get();

      if (perHourSnap.size >= MAX_BIDS_PER_HOUR) {
        return NextResponse.json({ 
          error: 'Too many negotiations. Please wait before trying again.',
          action: 'rate_limited',
          suspiciousFlag: true
        }, { status: 429 });
      }

      // Suspicious activity: bidding on >10 listings simultaneously
      const distinctListings = new Set(perHourSnap.docs.map(d => d.data().listingId));
      if (distinctListings.size > 10) {
        console.warn(`[SUSPICIOUS] Buyer ${buyerId} bidding on ${distinctListings.size} listings in 1 hour`);
      }
    } catch (rateErr) {
      // Don't block negotiation if rate limiting check fails
      console.warn('Rate limiting check failed:', rateErr);
    }

    // --- Log bid to history ---
    try {
      await firestore.collection('negotiation_history').add({
        buyerId,
        listingId,
        cropType,
        buyerOfferPerKg,
      ...
      console.warn('Failed to log bid:', logErr);
    }
