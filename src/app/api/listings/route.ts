...
export async function POST(req: Request) {
  try {
    ...
    // Server-side price validation: max 3x mandi price
    if (body.currentMandiPrice && body.askingPricePerKg > body.currentMandiPrice * 3) {
      return NextResponse.json({ 
        error: `Price too high. Mandi price is ₹${body.currentMandiPrice}/kg, maximum allowed: ₹${Math.round(body.currentMandiPrice * 3)}/kg` 
      }, { status: 400 });
    }
