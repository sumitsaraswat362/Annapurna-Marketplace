...
export default function BuyerMarketplace() {
  ...
  // Only show real listings from Firestore
  const activeListings = state.listings;
...
function ProduceCard({ listing, onNegotiate }: { listing: ProduceListing, onNegotiate: (qty: number) => void }) {
  ...
  const msSinceHarvest = Date.now() - listing.harvestDate;
  const minsSince = Math.floor(msSinceHarvest / (1000 * 60));
  const hrsSince = Math.floor(minsSince / 60);
  const daysSince = Math.floor(hrsSince / 24);
  const timeSinceHarvest = daysSince > 0 ? `${daysSince}d ago` : hrsSince > 0 ? `${hrsSince}h ${minsSince % 60}m` : minsSince > 0 ? `${minsSince}m ago` : 'Just now';
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 right-3 z-10">
        <span className="px-2.5 py-1 text-[10px] font-bold rounded-md bg-white/95 dark:bg-black/80 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-[var(--separator)] shadow-md flex items-center gap-1 backdrop-blur-md">
          <Clock className="w-3 h-3" /> {timeSinceHarvest}
        </span>
      </div>
